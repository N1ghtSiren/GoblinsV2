...
local function CheckFolderPath(path)
  messages.p("check folder path: %s", path)
  CreateFolder(path)
end
...
---@return boolean
local function WriteFile(path, filename, content)
  local file
  ...
  if(file == nil)then printd("file cannot be opened") return false end
...
saveloader_modules.driveAccessor = {
  WriteFile = WriteFile,
  ReadFile = ReadFile,
}